...
const REFUND_METHODS = ["CREDIT", "CASH", "TRANSFER", "CARD"] as const;
...
export function InvoiceReturnDialog({
  ...
  /** CREDIT | CASH | TRANSFER | CARD → the label in the reader's language. */
  const refundMethodLabel = (method: string) =>
    t(`invoiceReturn.method${method}`, { defaultValue: method });
  ...
  return (
    <div className="space-y-3 text-start" dir="rtl">
      ...
      >
        ...
        {t("invoiceReturn.open")}
      </Button>

      {previousReturns.length > 0 ? (
        <div className="rounded-xl border border-border/80 bg-muted/20 p-4">
          <p className="mb-2 text-sm font-semibold">{t("invoiceReturn.previousTitle")}</p>
          <ul className="space-y-2 text-sm">
            {previousReturns.map((r) => (
              <li key={r.id} className="rounded-lg border border-border/60 bg-card p-3">
                ...
                <p className="mt-1 text-xs">{t("invoiceReturn.previousReason", { reason: r.reason })}</p>
              </li>
            ))}
          </ul>
        </div>
      ...
      >
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg" dir="rtl">
          <DialogHeader>
            <DialogTitle>{t("invoiceReturn.title")}</DialogTitle>
            <DialogDescription>{t("invoiceReturn.description")}</DialogDescription>
          </DialogHeader>
          ...
          {created ? (
            <div className="space-y-4 py-2">
              <div className="rounded-md border border-green-300 bg-green-50 p-3 text-sm text-green-900 dark:border-green-800 dark:bg-green-950/30 dark:text-green-100">
                <p className="font-semibold">{t("invoiceReturn.successTitle")}</p>
                <p className="mt-1">
                  {t("invoiceReturn.successTotal")}{" "}
            ...
            <>
              <div className="space-y-4 py-2">
                <div className="overflow-x-auto rounded-lg border">
                  <table className="w-full text-sm">
                    <thead className="bg-muted/80 text-xs font-semibold">
                      <tr>
                        <th className="px-3 py-2 text-start">{t("invoiceReturn.colItem")}</th>
                        <th className="px-3 py-2 text-center">{t("invoiceReturn.colQty")}</th>
                        <th className="px-3 py-2 text-end">{t("invoiceReturn.colTotal")}</th>
                        <th className="px-3 py-2 text-center">{t("invoiceReturn.colReturnQty")}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {lines.map((line) => (
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">
                    {t("invoiceReturn.reasonLabel")} <span className="text-red-500">*</span>
                  </label>
                  <textarea
                    className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                    rows={3}
                    placeholder={t("invoiceReturn.reasonPlaceholder")}
                    value={reason}
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">{t("invoiceReturn.methodLabel")}</label>
                  <select
                    className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                    value={refundMethod}
              ...
              <DialogFooter className="gap-2 sm:justify-between">
                ...
                >
                  {createReturn.isPending ? t("invoiceReturn.submitting") : t("invoiceReturn.submit")}
                </Button>
              </DialogFooter>
              {createReturn.isError ? (
                <p className="text-sm text-destructive">
                  {getApiErrorMessage(createReturn.error, t("invoiceReturn.submitFailed"))}
                </p>
              ) : null}
            </>
          )}
